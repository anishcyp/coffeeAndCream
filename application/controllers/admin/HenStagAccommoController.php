...
class HenStagAccommoController extends BaseController
{
    ...
    function index()
    {
        $this->global['pageTitle'] = ' : Hen stag accommodation Listing';
        $this->loadViews(ADMIN."Manageaccommodation", $this->global, NULL, NULL);
    }
    ...
    function ajax_datatable(){
        ...
        $config['column_search'] = array('cus.fname','cus.lname');         
        //$config['order'] = array('cus.id' => 'desc');
        $config['order'] = array($fieldname => $_REQUEST['order'][0]['dir']);
        $config['custom_where'] = array('is_delete'=>0,'user_role'=>5,'accommodation_user'=>1);
        ...
        foreach ($records as $record) {
            ...
            <a onclick="return isConfirm()" href="'.ADMIN_LINK.'manage-hen-stag-accommodation/delete/'.$record->id.'" class="btn btn-icon waves-effect btn-danger"> <i class="fa fa-trash"></i></a>

            <a href="'.ADMIN_LINK.'manage-hen-stag-accommodation/gallery/'.$record->id.'" class="btn btn-icon waves-effect btn-info">Gallery</a>';
    ...
    function delete($id) 
    {
        ...
        redirect(ADMIN.'manage-hen-stag-accommodation');
    }
