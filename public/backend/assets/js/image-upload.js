    ...
    reader.onload = (function(theFile) {
      return function(e) {
        ...
        span.innerHTML = ['<div class="inner"><img class="thumb" src="', e.target.result,
                          '" title="', theFile.name, '"/><input type="checkbox" name="default_img" value="'+theFile.name+'" class="form-control default-img-radio"><span class="multi_remove_img" data-del="'+theFile.name+'"><i class="fa fa-trash" aria-hidden="true"></i></span></div>'].join('');
        ...
        if(uploaded_img_val=="")
        {
          $('#hidden_fi').val(theFile.name);
        }
        else
        {
          $('#hidden_fi').val(uploaded_img_val+","+theFile.name);
        }
        chk_img_val();
        ...
        $('.inner').on('change', function() {
          ...
          var get_del_img_nm = $(this).find('span.multi_remove_img').attr("data-del");

          $('input.default-img-radio').not($(this).find('input.default-img-radio')).prop('checked', false);
          $(".multi_remove_img").not($(this).find('input.default-img-radio')).css('display','block');

          if($(this).find('input.default-img-radio').prop("checked") == true)
          {
            $("#default_images_value").val(get_del_img_nm);
            $(this).find('.multi_remove_img').css('display','none');
          }
          else
          {
            $("#default_images_value").val('');
            $(this).find('.multi_remove_img').css('display','block');
          }
          chk_def_val();
        });
